classdef (Abstract)  inflatableobject < handle & dynamicprops
    ...
    properties (GetAccess = public, SetAccess=private)
        xyz1 %uninflated vertices
        xyz2 %inflated vertices
        inflated_xyz
    end

    properties (Access = protected,SetObservable)
        inflation_ = 1 %internal representation of inflation value
        parent_ = [] %parent object for linked inflatableobjects
    end
    ...
    methods
        ...
        function obj = inflatableobject(varargin)
            ...
            obj.internalUpdateInflation();
            obj.subscribeToInflationParent(@(x,y)obj.internalUpdateInflation);
        end
        ...
        function set(obj,propertyString,value)
            %SET - set object properties
            %usage:
            %set(inflatableHandle,'Inflation',1)
            obj.(propertyString)=value;
        end

        function value=get(obj,propertyString)
            %GET - get object property
            % inflation=get(inflationHandle,'Inflation');
